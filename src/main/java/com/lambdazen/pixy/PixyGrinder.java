...
public class PixyGrinder {
	...
	public PixyPipe convertQueryToPipe(String query, Object... params) {
		...
		// Visitor 1: Store the previous output named step, get the input named steps, clear if same. Gather dependent named steps.
	...
	public PixyPipe convertRuleToPixyPipe(String relName, List<PixyDatum> bindings) {
		PixyPipe ans = convertRuleToPixyPipe(relName, bindings, new VariableGenerator(), new HashMap<String, PixyDatum>());
		
		if (ans == null) {
			throw new PixyException(PixyErrorCodes.SEQUENCING_ERROR, "Encountered rule " + relName + " with bindings " + bindings);
		} else {
			return ans;
		}
	}
	
	private PixyPipe convertRuleToPixyPipe(String relName, List<PixyDatum> bindings, VariableGenerator varGen, Map<String, PixyDatum> replacements) {
		// Get signature
		...
		if (relationStack.contains(signature)) {
			throw new PixyException(PixyErrorCodes.RECURSION_NOT_SUPPORTED, "Recursion detected at " + signature + ". The evaluation stack has " + relationStack.toString());
		}
		...
		try {
			...
			if (relName.startsWith("not(") && relName.endsWith(")")) {
				...
				// Not clause looks like: __not_something(X_0, X_1) :- something(X_0, X_1), !, fail. __not_something(X_0, X_1).
				// The bindings are (with, params)
				PixyClause notClause1 = new PixyClause(new PixyDatum(PixyDatumType.RELATION, "__not_" + rootRelName, relTuple), notBody1);
				PixyClause notClause2 = new PixyClause(new PixyDatum(PixyDatumType.RELATION, "__not_" + rootRelName, relTuple), Collections.<PixyDatum>emptyList());
			...
			if (clauses == null) {
				throw new PixyException(PixyErrorCodes.REFERENCE_TO_MISSING_RULE, "Encountered reference to '" + signature + "'. Evaluation stack: " + relationStack);
			}
	...
	protected PixyPipe convertOrClausesToPixyPipe(List<PixyClause> clauses, List<PixyDatum> bindings, 
			VariableGenerator varGen, Map<String, PixyDatum> replacements) {
		// Go over the pipes
		...
		for (PixyClause clause : clauses) {
			...
			newBindings = renameOutputsIfNecessary(mergeResultingBindings, bindings, mergeVarsMap, newReplacements, varGen); 
	...
	private List<PixyDatum> renameOutputsIfNecessary(boolean rename, List<PixyDatum> bindings, Map<String, List<String>> mergeVarsMap, Map<String, PixyDatum> replacements, VariableGenerator varGen) {
	...
	private PixyPipe convertClauseToPipe(PixyClause clause, List<PixyDatum> bindings, VariableGenerator varGen, Map<String, PixyDatum> replacements) {
			...
			for (PixyDatum bodyItem : permute(permutation, clause.getBody(), numClausesByCuts)) {
				...
				case SPECIAL_ATOM:
					if (bodyItem.isFail()) {
						...
						throw new PixyException(PixyErrorCodes.INTERNAL_ERROR, "Unhandled special atom " + bodyItem);
					}
					break;
					
				case RELATION:
					step = convertRuleToPixyPipe(bodyItem.getRelationName(), bodyItem.getRelationTuple(), varGen, replacements);
					break;
			...
			if (!clause.getHead().getRelationName().startsWith("__not")) {
				for (PixyDatum binding : bindings) {
					...
					if (binding.getType() == PixyDatumType.VARIABLE) {
						...
						if (binding == null) {
							throw new PixyException(PixyErrorCodes.SEQUENCING_ERROR, "Variable " + varName + " can not be computed in the clause: " + clause + ". Replacements " + replacements);
						}
					}
				}
			}
	...
	private PixyPipe unifyVars(List<PixyDatum> bindings, List<PixyDatum> targets, PixyClause clause, Map<String, PixyDatum> replacements) {
		PixyPipe ans = null;
		int arity = bindings.size();
	...
	public static PixyPipe unifyVar(Map<String, PixyDatum> replacements,
			PixyPipe inputPipe, PixyDatum binding, PixyDatum target) {
		if (!target.isGround()) {
			...
			replacements.put(binding.getVarName(), new PixyDatum(PixyDatumType.SPECIAL_ATOM, "$" + binding.getVarName()));
